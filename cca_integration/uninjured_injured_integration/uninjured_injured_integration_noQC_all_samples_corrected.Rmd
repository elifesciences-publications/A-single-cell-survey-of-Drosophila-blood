...
reference.list <- pre_regress_all_data_list[c("B2","B4","Indrop_PC1","Indrop_PC2_1","B1","B3","Indrop_blood2_uninj","Indrop_blood3_uninj")
]
...
all_data.integrated <- RunPCA(object = all_data.integrated, npcs = 3, verbose = FALSE)
...
for (res in seq(0.1,2,0.1)){
  ...
  cat("\n##","Cluster resolution: ",res,"{.tabset}\n")
      all_data.integrated <- FindClusters(all_data.integrated, resolution = res,  verbose = FALSE)
      cat("\n### UMAP\n")
        p1 <- DimPlot(object = all_data.integrated, reduction = 'umap', split.by = "injured_st",  label = TRUE)
        print(p1)
        cat("\n")
      cat("\n### TSNE\n")
        p2 <- DimPlot(object = all_data.integrated, reduction = 'tsne', split.by = "injured_st", label = TRUE)
        print(p2)
        cat("\n")
  cat("\n")
}
