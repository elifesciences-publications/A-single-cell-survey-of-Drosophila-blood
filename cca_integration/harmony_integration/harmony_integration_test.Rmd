...
params:
    outputDir: "."
    vars_to_regress: !r c("percent.mito")
    resolution: 0.5
---
...
```{r}
# Create folders
resultsDir <- file.path(params$outputDir, "results_test",Sys.Date())
invisible(mapply(
    FUN = dir.create,
    list(resultsDir),
...
samples_list <- samples_list %>% purrr::map(function(x) {as.data.frame(x) %>% tibble::rownames_to_column(var = "gene_id")})


```

```{r}
all_data_matrix <- samples_list$selected_data_B2 %>% dplyr::full_join(samples_list$selected_data_B4, by = "gene_id") %>% 
    dplyr::full_join(samples_list$selected_data_PC1, by = "gene_id") %>% 
    dplyr::full_join(samples_list$selected_data_PC2_1, by = "gene_id") %>% 
    dplyr::full_join(samples_list$selected_data_B1, by = "gene_id") %>% 
    dplyr::full_join(samples_list$selected_data_B3, by = "gene_id") %>% 
    dplyr::full_join(samples_list$selected_data_Indrop_blood2_uninj, by = "gene_id") %>% 
    dplyr::full_join(samples_list$selected_data_Indrop_blood3_uninj, by = "gene_id") %>% 
    tibble::column_to_rownames(var = "gene_id") %>% as.matrix()
all_data_matrix[is.na(all_data_matrix)] <- "0"
```

```{r}
all_data_seurat <- CreateSeuratObject(counts=all_data_matrix, project = "drosophila", min.cells = 10, min.features = 100)
all_data_seurat[["percent.mt"]] <- PercentageFeatureSet(all_data_seurat, pattern = "^MT-")
all_data <- all_data_seurat %>% 
...
all_data <- all_data %>% 
    RunHarmony(c("tech"), plot_convergence = TRUE)
```
