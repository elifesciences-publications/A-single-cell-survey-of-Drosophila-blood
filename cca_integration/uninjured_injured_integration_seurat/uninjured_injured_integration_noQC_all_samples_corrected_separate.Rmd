...
We explore different levels of resolution.
```{r}
get_table <- function(object, res){
    column = paste0("integrated_snn_res.",res)
    table_clusters <- table( object@meta.data$injured_st,object@meta.data[,column]) %>% as.matrix() 
    uninj_inj_ratio <- apply(table_clusters,2,FUN = function(x){round(x[2]/x[1],2)})
    table_clusters <- rbind(table_clusters, uninj_inj_ratio)
...
for (res in seq(0.1,2,0.1)){
  ...
  cat("\n##","Cluster resolution: ",res,"{.tabset}\n")
      ...
      data_clusters <- get_table(all_data_full_.integrated, res)
      rio::export(data_clusters, file = file.path(resultsDir, paste0("prop_cells_",res,"_clusters.csv")), row.names = T)
      cat("\n### UMAP\n")
