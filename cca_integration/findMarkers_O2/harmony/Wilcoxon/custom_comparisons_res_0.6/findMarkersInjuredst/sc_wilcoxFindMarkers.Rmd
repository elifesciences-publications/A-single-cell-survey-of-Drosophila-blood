...
use_condaenv("bioPython", conda = "~/software/miniconda2/bin/conda", required = TRUE)
```
...
if(file.exists(file.path(resultsDir,paste0("res_",res,"_markers_customComparisons_conserved.Rda"))))
{
    load(file.path(resultsDir,paste0("res_",res,"_markers_customComparisons_conserved.Rda")))
}else{

markers_list_conserved <- list()
    
# 10 vs 6

markers <- FindConservedMarkers(all_data, ident.1 = 10, ident.2 = 6, grouping.var = "injured_st", verbose = FALSE)
markers <- ann_genes(markers,rows)
markers_list_conserved[[paste0("cluster_10vs6")]] <- markers

# 8 vs 13

markers <- FindConservedMarkers(all_data, ident.1 = 8, ident.2 = 13, grouping.var = "injured_st", verbose = FALSE)
markers <- ann_genes(markers,rows)
markers_list_conserved[[paste0("cluster_8vs13")]] <- markers

# 14 vs 1

markers <- FindConservedMarkers(all_data, ident.1 = 14, ident.2 = 1, grouping.var = "injured_st", verbose = FALSE)
markers <- ann_genes(markers,rows)
markers_list_conserved[[paste0("cluster_14vs1")]] <- markers


save(markers_list_conserved, file = file.path(resultsDir,paste0("res_",res,"_markers_customComparisons_conserved.Rda")))
}
```
...
if(file.exists(file.path(resultsDir,paste0("res_",res,"_markers_customComparisons.Rda"))))
{
    load(file.path(resultsDir,paste0("res_",res,"_markers_customComparisons.Rda")))
}else{
...
markers <- FindMarkers(all_data, ident.1 = 10, ident.2 = 6, verbose = FALSE)
markers <- ann_genes(markers,rows)
markers_list[[paste0("cluster_10vs6")]] <- markers

# 8 vs 13

markers <- FindMarkers(all_data, ident.1 = 8, ident.2 = 13, verbose = FALSE)
markers <- ann_genes(markers,rows)
markers_list[[paste0("cluster_8vs13")]] <- markers

# 14 vs 1

markers <- FindMarkers(all_data, ident.1 = 14, ident.2 = 1, verbose = FALSE)
markers <- ann_genes(markers,rows)
markers_list[[paste0("cluster_14vs1")]] <- markers


save(markers_list, file = file.path(resultsDir,paste0("res_",res,"_markers_customComparisons.Rda")))
}
```
...
for(cluster_n in names(markers_list)){
    ...
    cat("\n")
    cat("####",paste("Cluster",cluster_n,i,sep="-"),"\n")
    plot <- VlnPlot(all_data, features =  df_split[[i]]$gene_id,  group.by = "cluster", 
...
```{r plot_marker_genes_clusters_umap_conserved, results = "asis"}
plot_feature_plot_wrapper <- function(obj,sub_df){
    p <- FeaturePlot(obj, features = sub_df[1],cols = c("grey", "blue"), 
reduction = "umap") + 
        ggtitle(sub_df[1]) %>% invisible()
...
for(cluster_n in names(markers_list_conserved)){
    cat("\n")
    cat("### ",cluster_n," {.tabset} \n")
    df <- markers_list_conserved[[`cluster_n`]][1:min(12,dim(markers_list_conserved[[`cluster_n`]])[1]),]
    df_split <- split(df, ceiling(seq_along(df$gene_id)/6))
    for(i in names(df_split)){
        cat("\n")
        cat("####",paste("Cluster",cluster_n,i,sep="-"),"\n")
...
```{r plot_marker_genes_clusters_tsne_conserved, results = "asis"}
plot_feature_plot_wrapper <- function(obj,sub_df){
    p <- FeaturePlot(obj, features = sub_df[1],cols = c("grey", "blue"),
reduction = "tsne") +
        ggtitle(sub_df[1]) %>% invisible()
...
```{r plot_marker_genes_clusters_violin_conserved, results = "asis"}

for(cluster_n in names(markers_list_conserved)){
    cat("\n")
    cat("### ",cluster_n," {.tabset} \n")
    df <- markers_list_conserved[[`cluster_n`]][1:6,]
    df_split <- split(df, ceiling(seq_along(df$gene_id)/1))
    for(i in names(df_split)){
    cat("\n")
    cat("####",paste("Cluster",cluster_n,i,sep="-"),"\n")
