...
params:
    bcb_file: "data/2018-05-16/bcb.rda"
    min_umis: 100
    max_umis: !r Inf
    min_genes: 100
    max_genes: !r Inf
...
The sterile injury 2 and injured control 1 samples are pretty low quality, and most of the cells will be removed. We'll censor these samples from the final dataset.
...
min_genes <- c(
    "sterile injury 1"    = params$min_genes,
    "sterile injury 3"    = params$min_genes,
    "sterile injury 4"    = 650,
    "uninjured control 2" = params$min_genes,
    "uninjured control 3" = params$min_genes,
    "uninjured control 4" = params$min_genes,
    "uninjured control 5" = params$min_genes
)
...
# Stricter filtering by UMI knee point

```{r filter_cells_by_umi_knee}
bcb_filtered_knee <- bcb_original %>%
    selectSamples(sampleName = keep) %>%
    filterCells(
        minUMIs = "knee",
        minGenes = min_genes,
...
```{r plot_filtered_knee_qc, results="asis"}
plotQC(
    object = bcb_filtered_knee,
    interestingGroups = "sampleName",
    return = "markdown",
    headerLevel = 2
)
...
# Split high and low quality replicates

These samples have a much higher overall gene detection rate:
...
Luckily, there are at least 2 replicates from both conditions of interest, so we can use these as our high confidence set of samples.

```{r filtered_high_conf}
bcb_filtered_high_conf <- selectSamples(
    object = bcb_filtered_knee,
    sampleName = c(
        "sterile injury 1",
        "sterile injury 4",
    ...
    # Apply consistent 650 minimum gene cutoff (see above)
    filterCells(minGenes = 650)
```

The lower confidence replicates with poorer gene detection are:
...
```{r filtered_low_conf}
bcb_filtered_low_conf <- selectSamples(
    object = bcb_filtered_knee,
    sampleName = c(
        "sterile injury 3",
        "uninjured control 4",
