...
params:
    bcb_file: "data/2018-05-04/bcb.rda"
    min_genes: 100
    max_genes: !r Inf
    max_mito_ratio: 0.25
    min_novelty: 0.85
...
Note that the dataset has a lower than usual gene detection rate overall, which is likely biological. This seems to be fairly consistent across samples however, so we'll lower our gene threshold down to `r params$min_genes` genes per cell. Most of the cell filtering is comes at the novelty and mito steps instead (see below).

Sterile injury 2 has a very low gene detection rate, and it looks like we'll want to discard this sample from the final dataset.
...
Sterile injury 2 and injured control are pretty low quality, and most of the cells will be removed. We'll discard these from the final sample set.
...
We're splitting the FACS sorted sample out separately prior to filtering.

```{r facs}
facs <- selectSamples(bcb, facs = TRUE)
saveData(facs, dir = params$data_dir)
```
...
```{r censor_samples}
# Censoring sterile injury 2 and uninjured control 1.
# Also removing the FACS sample (see above).
censor <- c(
    "FACS sorted",
    "sterile injury 2",
    "uninjured control 1"
...
# Consider refiltering the cells per gene here
bcb_censor <- selectSamples(bcb, sampleName = keep)
